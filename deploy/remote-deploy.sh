...
setup_directories() {
    log_info "创建目录结构..."
    mkdir -p ${BACKEND_DIR} ${FRONTEND_DIR} ${LOGS_DIR} ${NGINX_CONF_DIR} ${BACKUP_DIR} ${SOURCE_DIR}
...
kill_port_processes() {
    ...
    # 查找占用指定端口的进程PID（精确匹配）
    local PIDS=$(netstat -tlnp 2>/dev/null | grep ":${APP_PORT}" | awk '{print $7}' | cut -d'/' -f1 | grep -E '^[0-9]+$' | sort -u)
    
    if [ -n "$PIDS" ]; then
        log_warn "发现占用端口 ${APP_PORT} 的进程: $PIDS"
        for PID in $PIDS; do
            # 获取进程详细信息，验证是否为Java进程（本项目）
            local CMDLINE=$(cat /proc/$PID/cmdline 2>/dev/null | tr '\0' ' ')
            if echo "$CMDLINE" | grep -q "todo-app\|java.*${APP_PORT}"; then
...
build_backend() {
    ...
    local backend_src="${SOURCE_DIR}/backend"
    
    if [ ! -d "${backend_src}" ]; then
        log_error "后端源码不存在: ${backend_src}"
        exit 1
    fi
    ...
    # 复制JAR到部署目录
    cp target/${JAR_NAME} ${BACKEND_DIR}/
    log_success "后端构建完成"
}
...
check_health() {
    ...
    local retry=0
    while [ $retry -lt 6 ]; do
        local http_code=$(curl -s -o /dev/null -w '%{http_code}' http://localhost:${APP_PORT}/api/todos 2>/dev/null || echo '000')
    ...
    cat > ${DEPLOY_RESULT_FILE} << RESULT
{
    ...
    "status": "${status}",
    "message": "${message}",
    "frontend_url": "http://$(hostname -I | awk '{print $1}'):${APP_PORT}",
    "backend_url": "http://$(hostname -I | awk '{print $1}'):${APP_PORT}/api/todos"
}
RESULT
    log_info "部署结果已记录"
}
...
full_deploy() {
    ...
    if check_health; then
        ...
        echo ""
        echo "访问地址:"
        echo "  前端: http://$(hostname -I | awk '{print $1}'):${APP_PORT}"
        echo "  后端: http://$(hostname -I | awk '{print $1}'):${APP_PORT}/api/todos"
        echo ""
...
show_status() {
    ...
    echo ""
    echo "端口监听:"
    netstat -tlnp 2>/dev/null | grep ":${APP_PORT}" || ss -tlnp | grep ":${APP_PORT}" || echo "无"
